# Hamed Abbasi — Web Developer

## About Me
I'm a web developer with 10+ years of experience in IT support and web development.
Based in Urmia, Iran. Open to remote opportunities.

## Skills
- WordPress (theme customization, plugins, WooCommerce)
- Joomla CMS
- Adobe Photoshop (UI design, banners, web graphics)
- C# (academic level)
- HTML / CSS
- IT Support & Helpdesk

## What I Can Do For You
- Build and customize WordPress websites
- Design professional graphics with Photoshop
- Fix WordPress issues and bugs
- Migrate websites to WordPress or Joomla

##Portfolio
- **saba.ac.ir** -Developed an maintained the official website of Saba Institutet, Urmia. Iran
- ![saba.ac.ir](./pic.jpg)

## Contact
- Email: hamed.abbasy@gmail.com
- Location: Urmia, Iran
